...
data_path = "./data/data/"
base_heights=[40,60,80,100]
diff=[-20,-15,-10,-5,0,5,10,15,20]

for bs in base_heights:
    for dis in diff:
        uav=bs+dis
        filename = data_path+str(bs)+'_'+str(uav)+'.txt'
        data_set = pd.read_table(filename,delimiter='\t')
        ...
        for mr in missing_rates:
            ...
            #    batch_xs, batch_ys = mnist.train.next_batch(100)
                ...
                _, accuracy,predict = sess.run([train_step,cross_entropy,prediction], feed_dict={xs: batch_ys, ys: batch_xs, keep_prob: 0.5})
                if i % 400 == 0:
                    print("base height:",bs,"uav:",uav,"miss rate",mr,"epoch:",i,"accuracy",accuracy)
#                    print("epoch:", i," mean square error", accuracy)
        ...
        accu.insert(0,uav)
        accu.insert(0,bs)
        out = open('data/plusmae.csv','a', newline='')
        ...
        ctime.insert(0,uav)
        ctime.insert(0,bs)
        out2 = open('data/plustime.csv','a', newline='')
